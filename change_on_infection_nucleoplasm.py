...
f=open("Processing/res_contrast_infected-nucleoplasm_vs_non_infected-nucleoplasm.csv").readlines()
diffExp_cytoplasm={}
for i in f[1:]:
    l=i.strip().split()
    gname=l[0].replace("\"","")
...
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.savefig("Overrepresented_gene_expression_nucleoplasm.pdf",format='pdf',dpi=300)
